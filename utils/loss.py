    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox, tbox[i], CIoU=True).squeeze()        # iou(prediction, target)  原先的iou

                # iou = bbox_iou(pbox, tbox[i], EIoU=True)                # 修改的 CIoU=True
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # Objectness
                # tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * iou.detach().clamp(0).type(tobj.dtype)  # iou ratio  torch版本高用
                tobj[b, a, gj, gi] = (1.0 - self.gr) + self.gr * torch.squeeze(iou.detach()).clamp(0).type(tobj.dtype)      # 修改 torch.squeeze(iou.detach())
