...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        # Loss
        if compute_loss:
            loss += compute_loss(train_out, targets)[1]  # 原先 box, obj
            # loss += compute_loss(train_out, targets, im)[1]  # 修改
...
def parse_opt():
    ...
    parser.add_argument('--task', default='test', help='train, val, test, speed or study')       # val >> test
