...
#--------------------------添加yolov6的 Efficient Decoupled Head 解耦头-------------------------------#
...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (Detect, Decoupled_Detect, Segment)):        # 修改 (Detect, Segment)-->(Detect, Decoupled_Detect, Segment)
...
class DetectionModel(BaseModel):
    ...
    #--------------------添加 Efficient Decoupled Head 解耦头--------------------------#
    def _initialize_biases(self, cf=None):  # initialize biases into Detect(), cf is class frequency
        # https://arxiv.org/abs/1708.02002 section 3.3
        # cf = torch.bincount(torch.tensor(np.concatenate(dataset.labels, 0)[:, 0]).long(), minlength=nc) + 1.
        m = self.model[-1]  # Detect() module
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='models/yolov5m_cota.yaml', help='model.yaml')
    parser.add_argument('--batch-size', type=int, default=1, help='total batch size for all GPUs')      # 可评价不同的batch size
    parser.add_argument('--device', default='0', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--profile', action='store_true', default='True', help='profile model speed')       # 添加了 default='True'
    parser.add_argument('--line-profile', action='store_true', default='True', help='profile model speed layer by layer')       # default设置为Flase则显示总时间，设置为True则显示每个层的时间
